...
**一本道**——テスト本体は上から下へ一直線に流れる。`if` / `switch` / 三項演算子 / `&&`・`||` の短絡が入った瞬間、そのテストは「どの枝を通ったか」を自分では保証できなくなり、テストの実装が正しいことを別の何かで証明する必要が出てくる——テストは検証する側であり、検証される側ではない。枝が欲しくなったらシナリオが2つあるということなので、テストを2つに割ること。
