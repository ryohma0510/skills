...
書き出したパスを渡して Skill ツールで `sanitize-doc` を発動する。整形後のファイルの内容を返信本文として使う。
...
完了条件: ステップ1で対象にしたスレッドとスレッド外のコメントのすべてに返信が付き、直したものの返信にはコミットへのリンクが入っていて、その本文は `sanitize-doc` を適用済みである。
