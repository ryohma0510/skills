...
今回の変更を対象に Skill ツールで `eli5` を発動する。得た説明を、本文の ELI5 の `<details>` に入れる。

以降のステップは、このファイルの内容を PR 本文として扱う。

完了条件: テンプレートの各セクションが埋まっているか意図的に省略されていて、その本文がファイルに保存され、`sanitize-doc` を適用済みであり、ELI5 の `<details>` に `eli5` の出力が入っている。
