...
| [implement](skills/implement/SKILL.md) | 実装作業の入り口。`tdd` でテストファーストに進め、`code-comments` でコメント品質を確認し、完了後に `subagent-review` でレビューする |
| [skill-design-principles](skills/skill-design-principles/SKILL.md) | Predictable なスキルを書くための語彙と原則。スキルの作成・編集・診断時に参照する |
| [subagent-review](skills/subagent-review/SKILL.md) | 変更差分をサブエージェントにレビューさせ、結果を深刻度順に整形して表示する |
